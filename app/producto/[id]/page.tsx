...
export default function ProductDetailPage({ params: paramsProp }: PageProps) {
  ...
  useEffect(() => {
    ...
    client.models.Product.get({ id: resolvedId })
      .then(({ data, errors }) => {
        if (!isMounted) return;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-black">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.back()}
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 text-xs sm:text-sm font-medium transition-all group hover:border-cyan-500/50 cursor-pointer"
            ...
            <Link href="/" className="flex items-center gap-2 group cursor-pointer focus:outline-none">
              ...
              <span className="text-lg sm:text-xl font-black tracking-widest text-white group-hover:text-cyan-400 transition-colors">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="text-xs sm:text-sm text-slate-400 hover:text-cyan-400 transition-colors font-medium hidden sm:inline"
            >
              Catálogo
            </Link>
            ...
            <button
              ...
              className="relative p-2 rounded-lg bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 transition-all group hover:border-cyan-500/50 hover:shadow-[0_0_12px_rgba(6,182,212,0.25)] cursor-pointer"
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
              ...
              <div className="w-full space-y-4">
                {/* Contenedor Principal de Imagen con Marco Sólido Técnico y Brillo Neón */}
                <div className="relative w-full aspect-square bg-slate-950 rounded-2xl border-2 border-cyan-500/40 overflow-hidden shadow-[0_0_30px_rgba(6,182,212,0.2),inset_0_0_20px_rgba(6,182,212,0.05)] group">
                  ...
                  {/* Renderizado de StorageImage Principal / Activa con loading=lazy y esquinas perfectamente redondeadas */}
                  {activeImage ? (
                    <StorageImage
                      key={activeImage}
                      path={activeImage}
                      ...
                      loading="lazy"
                      className="w-full h-full object-cover rounded-2xl transition-all duration-300 relative z-0"
                      fallbackSrc="/favicon.ico"
                    />
                  ) : (
                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-500 bg-slate-950 rounded-2xl">
                      ...
                      <span className="text-xs uppercase tracking-widest font-mono text-slate-400">NO_SIGNAL // SIN FOTO</span>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  <div className="p-3 rounded-xl bg-slate-900/80 border border-cyan-500/20 flex items-center gap-3 shadow-[0_0_10px_rgba(6,182,212,0.05)]">
                    ...
                    <div>
                      <p className="text-xs font-bold font-mono text-white">AUTÉNTICO // Y2K</p>
                      <p className="text-[11px] text-slate-400">Curaduría de archivo</p>
                    </div>
                  </div>

                  <div className="p-3 rounded-xl bg-slate-900/80 border border-purple-500/20 flex items-center gap-3 shadow-[0_0_10px_rgba(168,85,247,0.05)]">
                    ...
                    <div>
                      <p className="text-xs font-bold font-mono text-white">ENVÍO PRIORITARIO</p>
                      <p className="text-[11px] text-slate-400">Todo Chile express</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs font-mono font-bold tracking-wider uppercase px-3 py-1 rounded-full bg-cyan-950/90 text-cyan-300 border border-cyan-700/50 shadow-[0_0_10px_rgba(6,182,212,0.2)]">
                    CAT: {product.category || 'General'}
                  </span>
                  {product.gender && (
                    <span className="text-xs font-mono font-bold tracking-wider uppercase px-3 py-1 rounded-full bg-purple-950/90 text-purple-300 border border-purple-700/50 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
                      GEN: {product.gender}
                    </span>
                  )}
                  {product.brand && (
                    <span className="text-xs font-mono font-bold tracking-wider uppercase px-3 py-1 rounded-full bg-slate-800/90 text-amber-300 border border-amber-800/50">
                      BRD: {product.brand}
                    </span>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2 font-mono text-xs text-slate-500">
                    <span className="text-cyan-400/80">SYS://NODE_ID:</span>
                    <span className="text-slate-400 font-bold">{product.id}</span>
                  </div>
                </div>

                {/* Precio Grande con Estilo Monitor Neón */}
                <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/90 to-slate-950 border border-cyan-500/30 shadow-[0_0_20px_rgba(6,182,212,0.1)] flex items-baseline justify-between">
                  <div>
                    <span className="text-xs font-mono uppercase tracking-widest text-slate-400 block mb-1">VALOR UNITARIO</span>
                    <span className="text-3xl sm:text-4xl font-black font-mono text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-teal-300 to-emerald-400 drop-shadow-[0_0_12px_rgba(6,182,212,0.35)]">
                      ${formattedPrice} <span className="text-xs font-normal text-slate-400 font-sans">CLP</span>
                    </span>
                  </div>

                  <div className="text-right">
                    <span className="text-[11px] font-mono text-slate-400 block">ESTADO</span>
                    <span className={`text-xs font-mono font-bold ${availableStock > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                ...
                <div className="rounded-xl bg-slate-900/80 border border-cyan-500/20 p-5 space-y-4 shadow-[0_0_15px_rgba(6,182,212,0.05)]">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm font-mono">
                    ...
                    <div className="space-y-1">
                      <span className="text-[11px] text-slate-400 block">CATEGORÍA</span>
                      <span className="font-semibold text-white">{product.category || 'No especificada'}</span>
                    </div>

                    {/* Género */}
                    <div className="space-y-1">
                      <span className="text-[11px] text-slate-400 block">GÉNERO</span>
                      <span className="font-semibold text-white">{product.gender || 'Unisex'}</span>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="font-semibold text-cyan-300">{product.size || 'Única / Estándar'}</span>
                    </div>

                    {/* Stock */}
                    <div className="space-y-1">
                      <span className="text-[11px] text-slate-400 block">STOCK DISPONIBLE</span>
                      <span className="font-semibold text-white font-mono">{availableStock} unidades</span>
                    </div>
                    ...
                    {product.brand && (
                      <div className="space-y-1">
                        <span className="text-[11px] text-slate-400 block">MARCA</span>
                  ...
                  {product.description && (
                    <div className="pt-3 border-t border-slate-800/80">
                      <span className="text-[11px] text-slate-400 font-mono block mb-1">DESCRIPCIÓN TÉCNICA</span>
                ...
                <div className="space-y-4 pt-2">
                  {availableStock > 0 && (
                    <div className="flex items-center gap-4">
                      <span className="text-xs text-slate-400 font-mono">CANTIDAD:</span>
                      <div className="inline-flex items-center rounded-xl bg-slate-950 border border-cyan-500/30 p-1">
                        <button
                          type="button"
                          onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                          disabled={quantity <= 1}
                          className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-300 hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed text-base font-bold transition-colors cursor-pointer"
                        >
                          -
                        </button>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-slate-900 bg-slate-950/90 py-6 text-center text-xs text-slate-500 mt-auto">
