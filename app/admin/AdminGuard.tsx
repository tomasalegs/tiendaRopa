...
export function AdminRBACGuard({
  ...
  return <>{children(userRole || 'Admin')}</>;
}
...
export function AdminHeader({
    ...
    async function loadAttributes() {
      try {
        ...
        if (isMounted) {
          ...
          const firstName = fullName ? fullName.split(' ')[0] : (user?.signInDetails?.loginId ? user.signInDetails.loginId.split('@')[0] : 'Admin');
  ...
  const displayName = userName || user?.signInDetails?.loginId?.split('@')[0] || user?.username || 'Admin';

  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-slate-800/90">
      <div>
      ...
      <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-between md:justify-end">
        {user && (
          <span className="text-xs font-mono text-slate-300 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm">
          ...
          <span
            className={`text-xs font-mono font-bold px-3 py-1.5 rounded-lg border flex items-center gap-1.5 ${
              userRole === 'Super_Admin'
                ...
                ? 'bg-purple-950/80 text-purple-300 border-purple-700/60 shadow-[0_0_10px_rgba(168,85,247,0.3)]'
                : 'bg-cyan-950/80 text-cyan-300 border-cyan-700/60 shadow-[0_0_10px_rgba(6,182,212,0.3)]'
            }`}
          >
        ...
        {signOut && (
          <button
            onClick={signOut}
            className="bg-rose-950/80 hover:bg-rose-900 text-rose-300 hover:text-white border border-rose-800/60 px-3.5 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5 cursor-pointer shadow"
          >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-cyan-500 selection:text-black">
      ...
      <style jsx global>{`
        ...
        [data-amplify-tabs-item] {
          color: #94a3b8 !important;
          font-weight: 700 !important;
        }
        [data-amplify-tabs-item--active] {
        ...
        <Authenticator formFields={authFormFields}>
          {({ signOut, user }) => (
            <div className="w-full">
              <AdminRBACGuard signOut={signOut} user={user}>
                {(userRole) => children({ signOut, user, userRole })}
              </AdminRBACGuard>
            </div>
          )}
        </Authenticator>
